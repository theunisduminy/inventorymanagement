...
add_shortcode('product_cum_losses_table', function ($atts) {
  ob_start();
    $query = new WP_Query(array(
    'post_type' 		    => 'product',
    'posts_per_page' 	  => -1,
  ...
  if ($query->have_posts()) { ?>
    ...
    <tr>
  	...
  	<th style="text-align: left;">Cumulative Losses</th>
    </tr>
    </thead>
    <tbody>
  <?php while ($query->have_posts()) : $query->the_post(); ?>
...
// RM losses table
add_shortcode('rm_cum_losses_table', function ($atts) {
  ob_start();
    $query = new WP_Query(array(
    'post_type' 		    => 'raw_material',
    'posts_per_page' 	  => -1,
...
// Product losses table
add_shortcode('product_losses_table', function ($atts) {
  ob_start();
    $query = new WP_Query(array(
    'post_type' 		    => 'product',
    'posts_per_page' 	  => -1,
  ...
  <thead>
    <tr>
      ...
      <?php foreach ($stocktakes as $key => $stocktake) {
        echo '<th style="text-align: left;">'.$stocktake->post_title . ' losses' . '</th>';
      } ?>
    </tr>
  </thead>
  <tbody>
